...
$.ajax({
    type: 'POST',
    url: "${pageContext.request.contextPath}/calendar/getAllEvent",
    data:{
    	"roomname": roomname
    },
      ...
      //Drop =================================
        eventDrop: function(event, delta, revertFunc) {
        		...
        		if (event.username == username || chief == username) {
        			...
        			if(confirm("정말 변경하시겠습니까?")){
                      ...
                      $.ajax({
                          type: 'GET',
                          url: "${pageContext.request.contextPath}/calendar/modifyEvent",
                          data: {
                            "calnum": event.calnum,
                            "calevent": event.title,
                            "caldate1": event.start,
                            "caldate2": event.end
                          },
                          success: function(data) {
            					console.log(data);
        ...
        eventResize: function(event, delta, revertFunc) {
    		...
    		if (event.username == username || chief == username) {
    			...
    			if(confirm("정말 변경하시겠습니까?")){
                  ...
                  var caldate1 = new Date(event.start);
                  var caldate2 = new Date(event.end);
                  
                  var caldate1 = caldate1.getFullYear() + "-" +
                  			   (caldate1.getMonth() + 1) + "-" +
                  			   caldate1.getDate();
                  var caldate2 = caldate2.getFullYear() + "-" +
                  			   (caldate2.getMonth() + 1) + "-" +
                  			   caldate2.getDate();
             		...
             		element.find(".closeon").on('click', function(e) {
                		...
                		if (event.username == username || chief == username) {
                			...
                			if(confirm("정말 삭제하시겠습니까?")){
                                ...
                                $.ajax({
                                    type: 'POST',
                                    url: "${pageContext.request.contextPath}/calendar/removeEvent",
                                    data: {
                                      "calnum": event.calnum,
                                      "calevent": event.title,
                                      "caldate1": caldate1,
        ...
        select: function(start, end, jsEvent, view) {
          ...
          if (title != null) {
            ...
            var caldate1 = new Date(start);
            var caldate2 = new Date(end);
            ...
            $.ajax({
              ...
              data: {
                "calnum": event.calnum,
                "calevent": title.trim() != "" ? title : "New event",
                "caldate1": caldate1,
                "caldate2": caldate2,
                "roomname": roomname,
                "member_id": username
